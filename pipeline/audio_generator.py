...
_VOICE_MAP = {
    ...
    "shqip": ("sq-AL-AnilaNeural", "sq-AL-IlirNeural"),
    "english": ("en-US-EmmaNeural", "en-US-GuyNeural"),
}
_DEFAULT_VOICE = ("en-US-EmmaNeural", "en-US-GuyNeural")
...
class AudioGenerator:
    def __init__(self, language: str = "Albanian"):
        ...
        self.tts_retries = AUDIO_CONFIG.get("tts_retries", 2)
        self.tts_retry_delay_sec = AUDIO_CONFIG.get("tts_retry_delay_sec", 1.5)
        self._load_model()
    ...
    def generate(
        ...
        narration_paths, scene_durations = self._generate_all_narration(scenes, output_dir, gender)
        total_narration_duration = sum(scene_durations)

        narration_full_path = output_dir / "narration_full.wav"
        self._concatenate_audio(narration_paths, narration_full_path, gap_sec=gap_sec)
        ...
        story["total_duration_sec"] = total_narration_duration

        logger.success(f"Final combined audio production bundle outputted to: {final_path} ({total_narration_duration:.1f}s)")
        return final_path

    def _generate_all_narration(
        self, scenes: List[dict], output_dir: Path, gender: Optional[str]
    ...
    def _synthesize_with_retries(self, text: str, output_path: Path, gender: Optional[str]) -> float:
        last_error = None
        for attempt in range(1, self.tts_retries + 2):
            try:
                asyncio.run(self._generate_edge_tts(text, output_path, gender))
                return self._get_wav_duration(output_path)
            except Exception as e:
                last_error = e
                logger.debug(f"TTS attempt {attempt} failed: {e}")
                if attempt <= self.tts_retries:
                    time.sleep(self.tts_retry_delay_sec)

        logger.warning(f"TTS engine failed after {self.tts_retries + 1} attempts ({last_error}). Generating fallback silence.")
        fallback_duration = 4.0
        self._generate_silence(output_path, fallback_duration)
        return fallback_duration

    async def _generate_edge_tts(self, text: str, output_path: Path, gender: Optional[str]) -> None:
        import edge_tts

        voice = self._resolve_voice(gender)
        rate_str = self._speed_to_rate_string(AUDIO_CONFIG.get("tts_speed", 1.0))

        communicate = edge_tts.Communicate(text, voice, rate=rate_str)
        await communicate.save(str(output_path))

    def _resolve_voice(self, gender: Optional[str]) -> str:
        override = AUDIO_CONFIG.get("tts_voice_override")
        if override:
            return override

        female_voice, male_voice = _VOICE_MAP.get(self.language.lower(), _DEFAULT_VOICE)
        if gender and gender.lower() in ("male", "boy", "m"):
            return male_voice
        return female_voice
    ...
    def _generate_full_soundtrack(
        ...
        for i, (scene, dur) in enumerate(zip(scenes, scene_durations)):
            ...
            segment_dur = dur + (outro_tail_sec if is_last else gap_sec)
            mood = str(scene.get("mood", "happy")).lower()
            if mood not in _SCALES:
                mood = "happy"

            segment = self._generate_mood_segment(mood, segment_dur)
            segment = self._apply_edge_fades(segment, sr, fade_sec=0.25)
            segments.append(segment)

        full_track = np.concatenate(segments) if segments else np.zeros(sr * 2, dtype=np.float32)

        full_track = self._apply_fade(full_track, sr, fade_in_sec=1.5, fade_out_sec=3.0)
        full_track = self._normalize(full_track, target_peak=AUDIO_CONFIG.get("music_volume", 0.3))
        self._save_wav(full_track, output_path, sr)

    def _generate_mood_segment(self, mood: str, duration_sec: float) -> np.ndarray:
        sr = self.sample_rate
        n = max(1, int(sr * duration_sec))
        t = np.linspace(0, duration_sec, n, endpoint=False)

        melody = self._compose_melody(t, mood, sr)
        harmony = self._compose_harmony(t, mood)
        bass = self._compose_bass(t, mood)
        reverb = self._apply_simple_reverb(melody, sr, delay_ms=80)

        return melody * 0.45 + harmony * 0.25 + bass * 0.15 + reverb * 0.15

    def _apply_edge_fades(self, signal: np.ndarray, sr: int, fade_sec: float = 0.25) -> np.ndarray:
        result = signal.copy()
        n = min(int(sr * fade_sec), len(result) // 4)
        if n <= 0:
            return result
        fade_curve = np.linspace(0.0, 1.0, n)
        result[:n] *= fade_curve
        result[-n:] *= fade_curve[::-1]
        return result

    def _compose_melody(self, t: np.ndarray, mood: str, sr: int) -> np.ndarray:
        scale = _SCALES.get(mood, _SCALES["happy"])
        tempo = _TEMPOS.get(mood, 100)
        beat = 60.0 / tempo
        total_dur = t[-1] + 1.0 / sr if len(t) else 0.0
        signal = np.zeros_like(t)

        rng = np.random.default_rng(seed=42)
        n_notes = int(total_dur / beat) + 1 if beat > 0 else 0
    ...
    def _compose_harmony(self, t: np.ndarray, mood: str) -> np.ndarray:
        chords = _CHORD_SETS.get(mood, _CHORD_SETS["happy"])
        signal = np.zeros_like(t)
        beat_dur = 2.0

        for i, chord in enumerate(chords * 100):
            start = i * beat_dur
            if len(t) == 0 or start >= t[-1]:
                break
    ...
    def _compose_bass(self, t: np.ndarray, mood: str) -> np.ndarray:
        notes = _BASS_NOTES.get(mood, _BASS_NOTES["happy"])
        signal = np.zeros_like(t)
        dur = 1.5
    ...
    def _normalize(self, signal: np.ndarray, target_peak: float = 0.8) -> np.ndarray:
        peak = np.max(np.abs(signal)) if len(signal) else 0.0
        if peak > 1e-6:
    ...
    def _mix_audio(self, narration_path: Path, music_path: Path, output_path: Path) -> None:
        ...
        music_ducked = self._duck_music(narration, music, sr)

        mixed = narration * voice_vol + music_ducked * music_vol
        mixed = self._normalize(mixed, target_peak=0.85)

        self._save_wav(mixed, output_path, sr)

    def _duck_music(self, narration: np.ndarray, music: np.ndarray, sr: int, duck_floor: float = 0.35) -> np.ndarray:
        if len(narration) == 0:
            return music

        window = max(1, int(sr * 0.03))
        abs_narration = np.abs(narration)

        cumsum = np.cumsum(np.insert(abs_narration, 0, 0.0))
        envelope = (cumsum[window:] - cumsum[:-window]) / window
        envelope = np.pad(envelope, (0, len(narration) - len(envelope)), mode="edge")

        peak = np.max(envelope) if len(envelope) else 0.0
        if peak <= 1e-6:
            return music

        envelope_norm = np.clip(envelope / peak, 0.0, 1.0)

        smoothed = np.copy(envelope_norm)
        alpha = 0.002
        for i in range(1, len(smoothed)):
            smoothed[i] = smoothed[i - 1] + alpha * (envelope_norm[i] - smoothed[i - 1])

        gain = 1.0 - (1.0 - duck_floor) * smoothed
        return music * gain
