...
class FaceProcessor:
    ...
    def _load_model(self) -> None:
        try:
            ...
            logger.success("InsightFace u ngarkua me sukses")

        except ImportError:
            raise FaceProcessingError(
                "insightface nuk është instaluar. "
                "Ekzekuto: pip install insightface"
            )
        except Exception as e:
            raise FaceProcessingError(f"Gabim duke ngarkuar InsightFace: {e}")
    ...
    def process(
        ...
        logger.debug(f"Duke procesuar foton: {photo_path}")

        image = self._load_image(photo_path)
        h, w  = image.shape[:2]
        logger.debug(f"Imazhi u lexua | dimensione: {w}x{h}px")
        ...
        logger.debug(f"Embedding u nxorr | dimensione: {embedding.shape}")

        augmented_paths = self._augment_face(
            face_image=face_image,
            temp_dir=temp_dir,
        )
        logger.debug(f"Augmentation | {len(augmented_paths)} imazhe u gjeneruan")
        ...
        try:
            ...
            if img is None:
                raise InvalidPhotoError(
                    "Imazhi nuk mund të lexohet. "
                    "Sigurohu që file-i nuk është i korruptuar.",
            ...
            raise FaceProcessingError(f"Gabim duke lexuar imazhin: {e}")
    ...
    def _detect_faces(self, image: np.ndarray) -> list:
        try:
            ...
            raise FaceProcessingError(f"Gabim gjatë detektimit: {e}")
        ...
        if not faces:
            logger.debug("Asnjë fytyrë nuk u gjet, duke provuar me imazh të zmadhuar...")
            faces = self._detect_with_upscale(image)

        if not faces:
            raise FaceNotDetectedError()

        logger.debug(f"{len(faces)} fytyrë/a u gjet/ën")
        return faces
    ...
    def _select_best_face(self, faces: list):
        ...
        logger.debug(
            f"{len(faces)} fytyra u gjetën, "
            f"u zgjodh më e madhja me score: {best.det_score:.3f}"
        )
        return best
    ...
    def _get_aligned_face(
        ...
        try:
            ...
            logger.debug(f"Face alignment deshtoi (opsionale): {e}")
            return None

    def _extract_embedding(self, face) -> np.ndarray:
        if face.embedding is None:
            raise FaceProcessingError(
                "Embedding nuk u gjenerua. "
                "Sigurohu që modeli buffalo_l është i instaluar plotësisht."
            )
    ...
    def _augment_face(
        ...
        try:
            ...
            logger.debug(f"Disa augmentations deshtuan (opsionale): {e}")
    ...
    def _save_image(self, image: np.ndarray, path: Path) -> None:
        ...
        if not success:
            raise FaceProcessingError(f"Nuk u ruajt imazhi: {path}")
